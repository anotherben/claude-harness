...
description: >
  ...
  missing; or when proof must span the diagnose post-fix mode, prove-it, PR checks, review
...
Turn completion claims into a proof-state verdict. This skill wraps `/diagnose` post-fix verification mode (formerly `patch-or-fix`), `prove-it`, and PR/deploy closeout checks instead of duplicating their detailed workflows.

## Required Skill Reuse

- Run `/diagnose` in post-fix verification mode when code, a fix, a PR diff, or a proposed repair exists and root-cause quality must be judged.
...
- `PATCH_OR_FIX_FAILED`: the post-fix verification found symptom patch, ownership/SRP/domain gap, or unproven root cause.
...
3. Run or require `/diagnose` post-fix verification mode for root-cause quality when code changed.
...
If either the post-fix verification mode or `prove-it` is required but unavailable, the proof status is no higher than `BLOCKED` or `PARTIALLY_PROVED`.
