...
Check for enterprise skills through the active Codex-local skill root or `skills-index`:
...
- **Found**: PASS
- **Not found**: MANUAL — "Enterprise skills not found at ~/.codex/skills/enterprise/. Resolve through skills-index or install from the reviewed skills snapshot."
...
Verify the engine works by calling `mcp__skills_index__skill_status()`. If it returns counts and freshness data, the index is live.
